...
**Status 2026-07-04: COMPLETE.** Two bugs found and fixed: (a) ST's VL53L4CX
API takes 8-bit addresses — passing 7-bit 0x29 had silently moved the sensor
to 0x14; now uses the 0x52 default; (b) XSHUT was soldered to D2 (strap pin)
while code drove GPIO4 — rewired to D4 and re-verified working.
**Field caveat:** the sensor is active 940 nm IR — matte-black/IR-absorbing
surfaces (e.g. a black desk) return too little signal → invalid range →
`tofValid=false` → FC falls back to baro (by design). Expect reduced range
over dark asphalt/carpet; grass/pavement are fine.
