...
def board_vertices(roll_r, pitch_r, yaw_r):
    length, width, t = 1.6, 1.0, 0.05
    corners = np.array([
        [-length/2, -width/2, 0], [ length/2, -width/2, 0],
        [ length/2,  width/2, 0], [-length/2,  width/2, 0],
        [-length/2, -width/2, t], [ length/2, -width/2, t],
        [ length/2,  width/2, t], [-length/2,  width/2, t],
    ], dtype=float)
    ...
    Rx = np.array([[1,0,0],[0,cr,-sr],[0,sr,cr]])   # roll around X (nose axis)
    Ry = np.array([[cp,0,sp],[0,1,0],[-sp,0,cp]])   # pitch around Y (tips nose up/down)
...
def update(_):
    ...
    # use same rotation matrix as board
    ...
    qx = ax.quiver(0,0,0,*x_axis,color='red')
    qy = ax.quiver(0,0,0,*y_axis,color='green')
    qz = ax.quiver(0,0,0,*z_axis,color='blue')
    ...
    fwd = np.array([0.8, 0, 0.025])                                                        # X is nose
